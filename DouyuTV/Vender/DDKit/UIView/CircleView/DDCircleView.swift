...
@objc public protocol DDCircleViewDelegate: NSObjectProtocol {
    ...
    //视图的点击回调
   optional func circleView(circleView: DDCircleView, didSelectedIndex: Int);
...
public class DDCircleView: UIView {
    ...
    public weak var circleDelegate: DDCircleViewDelegate?{
        ...
        didSet {
            ...
            self.imageCount = (circleDelegate?.imagesOfCircleView(self).count)!
            self.imageWithURLs = (circleDelegate?.imagesOfCircleView(self))!
            
            self.layoutIfNeeded()
            if imageCount <= 1 {
                
                
                 self.lastImageView.setImageWithURL(self.imageWithURLs[0] as! NSString)
            } else {
...
//        contentScrollView.backgroundColor = UIColor.greenColor()
    ...
    public override func layoutSubviews() {
       ...
       self.contentScrollView.contentSize =  CGSizeMake( self.bounds.width * 3, self.frame.size.height)
    ...
    func imageTapAction(tap: UITapGestureRecognizer){
        ...
        print("selected \(indexOfCurrentImage)")
        if let delegate = self.circleDelegate {
            
            delegate.circleView!(self, didSelectedIndex: indexOfCurrentImage);
        }
